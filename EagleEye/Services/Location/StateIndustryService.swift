//
//  StateIndustryService.swift
//  EagleEye
//
//  Looks up a state's top employing industries from the Census Bureau's free
//  ACS 5-year "Industry by Occupation" subject table (S2403, no API key
//  required), mirroring `DistrictIndustryService`'s use of the same table but
//  queried at the state level instead of per congressional district.
//

import Foundation

struct StateIndustryService {
    ...
    /// The employed civilian population's top industries in a state, most-
    /// to least-employing, or `nil` if the state has no FIPS code on file
    /// (the territories) or the API has no matching row. `limit` caps how
    /// many industries are returned.
    func topIndustries(state: String, limit: Int = 3) async throws -> [String]? {
